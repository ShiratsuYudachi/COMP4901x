...
void GameEngine::activate_monsters()
{
    ...
    {
        if ((py + dy < 0 || py + dy >= MAP_SIZE) || (px + dx < 0 || px + dx >= MAP_SIZE))
            continue;
        if (map[px+dx][py+dy]->get_unit_type() == UnitType::MONSTER)
            activate_monster((Monster *)map[px+dx][py+dy]);
    }
}
...
void GameEngine::player_discover()
{
    int px, py;
    player->get_position(px, py);
    for (int dx = -2, dy = -2; dx < 3 && dy < 3; dx = dx<3? dx+1: -1,dy+= dx<3? 0: 1)
    {
        if ((py + dy < 0 || py + dy >= MAP_SIZE) || (px + dx < 0 || px + dx >= MAP_SIZE))
            continue;
        map[px+dx][py+dy]->set_discovered();
    }
}

void GameEngine::processdir(int &dx,int &dy,int &nx,int &ny, Direction dir, int range)
{
    switch (dir)
    {
    case Direction::UP:
...
void GameEngine::player_attack()
{
    ...
    player->get_position(px, py);
    processdir(dx,dy,nx,ny,player->get_direction(),player->get_range());
